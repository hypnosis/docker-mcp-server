...
- [ ] Создать `src/managers/env-manager.ts`
- [ ] Реализовать чтение .env файлов (.env, .env.local, .env.development)
- [ ] Интегрировать библиотеку `dotenv`
- [ ] Мержить несколько .env файлов в правильном порядке
- [ ] Поддержать переменные из docker-compose.yml
...
- [ ] Создать `src/security/secrets-masker.ts`
- [ ] Реализовать маскирование секретов по keywords
- [ ] Keywords: PASSWORD, TOKEN, KEY, SECRET, API_KEY, PRIVATE, CREDENTIALS
- [ ] Заменить значения на `***MASKED***`
- [ ] Добавить опцию отключения маскирования
...
- [ ] Создать `src/security/sql-validator.ts`
- [ ] Реализовать проверку опасных SQL паттернов
- [ ] Паттерны: DROP DATABASE, DELETE без WHERE, TRUNCATE, DROP TABLE
- [ ] Добавить включение через `DOCKER_MCP_VALIDATE_SQL` env var
- [ ] Интегрировать в database tools (опционально)
